...
function EqSlot({ id, select, selected }: { id: number; select: Function; selected: number | undefined }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      {id === selected && (
        <Modal mousePosX={modal.x} mousePosY={modal.y} close={() => select(undefined)} slotId={id} where="equipments" />
      )}
      <div
...
function Modal({
  ...
  // In case the player click outside the modal
  useEffect(() => {
  ...
  const actions = () => (
    <>
      {/* USE ITEM */}
      {
        item?.type === "consumable" && (<button>Use</button>)
      }

      {/* EQUIP ITEM */}
      {
        (item?.type === "weapon" || item?.type === "armor")  && where !== "equipments" && (
          <button
            onClick={() => {
              inv.equipItem(item, slotId);
              close();
            }}>
            Equip
          </button>
      ...
      {/* INFO */}
      <button onClick={handleDesc}>Info</button>
  ...
  const description = () => (
    <>
      ...
      <p>{item?.subDescription && item?.subDescription}</p>
    </>
  );

  return createPortal(
    <div ref={modalRef} className={styles.modal} style={{ top: mousePosY, left: mousePosX }}>
      {modalType == "actions" ? actions() : description()}
    </div>,
    document.body
  );
}
